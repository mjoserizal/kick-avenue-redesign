      ...
      >
        {loopCards.map((card, index) => {
          ...
          return (
            ...
            >
              <span className={`flex size-11 shrink-0 items-center justify-center rounded-full shadow-sm ${card.iconSurface} ${card.accent}`}>
                <Icon className="size-5" strokeWidth={1.7} />
              </span>
              <div>
                <h3 className="text-sm font-bold text-neutral-900">{card.title}</h3>
                <p className="mt-1 text-xs leading-5 text-neutral-600">{card.copy}</p>
              </div>
            </div>
          );
        })}
